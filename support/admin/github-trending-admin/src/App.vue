...
import { Activity, CalendarClock, CheckCircle2, CircleAlert, Database, Eye, FileText, Github, LoaderCircle, Play, RefreshCw, Save, Settings2, Timer, X, XCircle } from "lucide-vue-next";
...
<template>
  <main class="app-shell">
    ...
    <section class="panel table-panel"><div class="panel-heading snapshot-heading"><div><p class="eyebrow">数据浏览</p><h2>Trending 快照</h2></div><label class="date-filter">按日期<input v-model="snapshotDate" type="date" @change="refreshSnapshots" /></label></div><div class="table-wrap"><table><thead><tr><th>#</th><th>仓库</th><th>语言</th><th>总 Stars</th><th>今日新增</th><th>README</th></tr></thead><tbody><tr v-for="item in snapshots" :key="item.fullName"><td class="rank">{{ item.rank }}</td><td><strong class="repository-name">{{ item.fullName }}</strong><small>{{ item.description || "无描述" }}</small></td><td>{{ item.language || "-" }}</td><td>{{ item.totalStars?.toLocaleString() || "-" }}</td><td>{{ item.starsToday?.toLocaleString() || "-" }}</td><td><button class="readme-button" title="在管理页查看 README" @click="openReadme(item)"><Eye :size="14" /> 查看 <span :class="item.readmeError ? 'readme-failed' : 'readme-ok'">{{ item.readmeError ? "失败" : item.hasReadme ? "已获取" : "无内容" }}</span></button></td></tr><tr v-if="!snapshots.length"><td colspan="6" class="empty">该日期暂无快照</td></tr></tbody></table></div></section>
